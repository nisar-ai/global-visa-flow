...
function sanitizeIdPart(str: string) {
...
export function Checklist({
  ...
  const state =
    documents.length > 0 && checked.length === documents.length
      ? checked
      : new Array(documents.length).fill(false);

  const doneCount = state.filter(Boolean).length;
  const progressPct =
    documents.length === 0 ? 0 : (doneCount / documents.length) * 100;
  ...
  const baseId = `checklist-${sanitizeIdPart(checklistKey)}`;
    ...
    <div className="mt-7">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        ...
        <p className="font-mono text-xs text-[var(--text-muted)]">
          {hydrated
            ? t.checklist_progress(doneCount, documents.length)
            : t.checklist_progress(0, documents.length)}
        </p>
      </div>
      ...
      <div
        className="mt-3 h-1.5 overflow-hidden rounded-full border border-[var(--border)] bg-[var(--surface-2)]"
        aria-hidden="true"
      >
        <div
